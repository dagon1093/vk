...
check: function(iUpload) {
  ...
  if (options.signed) {
    ...
    if (options.check_aid) {
      check_vars.aid = options.check_aid;
    }
    if (options.check_gid) {
      check_vars.gid = options.check_gid;
    }
...
serverFail: function(iUpload, err_obj) {
  ...
  if (options.signed) {
    var parts = Upload.uploadUrls[iUpload].split('?'), tmp = AjaxConvert.fromQueryString(parts[1]), q = extend({
      act: 'check_result',
  ...
  var query = extend({
    ...
    st2_server: options.st2_server,
    error: options.error,
    hash: options.error_hash,
    hash_extra: vars.hash_extra
  }, err_obj);
  ...
  if (vars.transform_entry) {
    extend(query, {
      transform_entry: vars.transform_entry,
      transform_hash:  vars.transform_hash,
      security_hash:   vars.security_hash
    });
  }
  if (window.ajax) {
